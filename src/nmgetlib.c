...
open_lib(struct lib_struct *lib_p, int ldnaseq, int *sascii, int outtty){
	...
	lib_type=0;
	if ((bp=strchr(lib_p->file_name,' '))!=NULL || (bp=strchr(lib_p->file_name,'^'))!=NULL) {
...
struct lmf_str * re_openlib(struct lmf_str *om_fptr, struct lib_struct *lib_p, int outtty) {
	int opnflg;
...
int agetlib(unsigned char *seq, int maxs,
