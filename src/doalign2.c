...
struct a_struct  doalign (FILE *fp, unsigned char **aa0, unsigned char *aa1save, int maxn,
		...
		)
{
	...
	for (ib=0; ib<istop; ib++) {
		...
		while (cur_ares_p != NULL && cur_ares_p->nres > 0) {
			...
			printf("nnidenty: %i\n", l_aln_p->nident);
			l_aln.pid = calc_fpercent_id(100.0, l_aln_p->nident,lc,m_msp->tot_ident, -1.0);
